...
/// <summary>
/// Presentation controller used only by TL_Intro_Capture. Timeline owns actor
/// animation and camera activation while this component evaluates editable
/// world-space motion/rotation cues, subtitles, fades and optional audio.
/// </summary>
[ExecuteAlways]
[DisallowMultipleComponent]
public sealed class IntroCaptureCutsceneController : MonoBehaviour
{
    [Serializable]
    ...
    public struct CameraCue
    {
        ...
        [Tooltip("Use world-space Euler rotations instead of automatically following Look Target.")]
        public bool useManualRotation;

        [Tooltip("World-space camera rotation at the beginning of the cue.")]
        public Vector3 startEulerAngles;

        [Tooltip("World-space camera rotation at the end of the cue.")]
    ...
    [SerializeField, Min(0.01f)] private float openingFadeDuration = 0.65f;
    [SerializeField] private double closingFadeStart = 29.5d;
    [SerializeField] private double closingFadeEnd = 32d;
    ...
    private void UpdateActors(double time)
    {
        for (int i = 0; i < actorMotions.Length; i++)
        {
            ...
            bool shouldFollowTerrain = firstCue.followTerrain;
            ...
            for (int j = i; j < actorMotions.Length; j++)
            {
                ...
                shouldFollowTerrain = cue.followTerrain;
                if (time <= cue.end)
                {
                    float progress = Mathf.SmoothStep(0f, 1f, NormalizedTime(time, cue.start, cue.end));
                    position = Vector3.LerpUnclamped(cue.startPosition, cue.endPosition, progress);
                    break;
                }
            ...
            if (shouldFollowTerrain && TrySampleTerrainHeight(position, out float terrainHeight))
            {
            ...
            {
                Vector3 flatDirection = Vector3.ProjectOnPlane(facingDirection, Vector3.up);
                if (flatDirection.sqrMagnitude > 0.0001f)
                {
                    marker.rotation = Quaternion.LookRotation(flatDirection.normalized, Vector3.up);
                }
            }
        }
    }
