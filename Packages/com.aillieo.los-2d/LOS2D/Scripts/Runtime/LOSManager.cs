...
namespace AillieoUtils.LOS2D
{
    public class LOSManager : MonoBehaviour
    {
        ...
        /// <summary>
        /// ASTRA EDEN fork: default upstream was hardcoded 1m — too tight for 3D dinosaurs/player.
        /// Vertical |target.y - source.y| must be &lt;= this to count as in sector.
        /// </summary>
        public static float heightTolerance = 8f;
            ...
            foreach (var t in targetBuffer)
            {
                foreach (var s in sourceBuffer)
                {
                    ...
                    if (InSector(t, s))
                    {
                        ...
                        sPos.y += s.eyeHeight;
                        // Aim chest-ish (target pivot + small up) so ray doesn't skim ground.
                        Vector3 tPos = t.transform.position + Vector3.up * 1f;
                        Vector3 dir = tPos - sPos;
                        float dist = dir.magnitude;
                        if (dist > 0.001f && dist <= s.maxDist)
                        {
                            Ray ray = new Ray(sPos, dir.normalized);
                            // ASTRA patch: hit target itself OR child/parent of target (upstream IsChildOf only failed on self-hit).
                            if (Physics.Raycast(ray, out RaycastHit hit, dist, s.maskForEvent, QueryTriggerInteraction.Ignore))
                            {
                                Transform ht = hit.transform;
                                if (ht == t.transform
                                    || ht.IsChildOf(t.transform)
                                    || t.transform.IsChildOf(ht)
                                    || ht.root == t.transform.root)
                                {
                                    inSight = true;
                                }
                                // else: hit wall/obstacle first → not in sight
                            }
                            // miss entirely (no collider): treat as blocked for stealth fairness in open maps with thin walls
                        }
                    }
        ...
        private static bool InSector(LOSTarget target, LOSSource source)
        {
